...
output "lb_id" {
  ...
  value       = var.create_load_balancer ? ibm_is_lb.lbs.*.id : data.ibm_is_lb.lbs.*.id
}

output "lb_hostname" {
  description = "The ID of the Load balancer"
  value       = var.create_load_balancer ? ibm_is_lb.lbs.*.hostname : data.ibm_is_lb.lbs.*.hostname
}
